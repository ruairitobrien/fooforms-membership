        ...
        organisationCommand.createOrganisation(organisationDetails, function (err, result) {
            if (err || result.err || !result.organisation) {
                ...
                teamCommand.createTeam({
                    displayName: organisation.displayName + '-owners',
                    title: organisation.title + ' Owners',
                    description: 'Owners of ' + organisation.title,
                    ...
                    teamCommand.createTeam({
                        displayName: organisation.displayName + '-members',
                        title: organisation.title + ' Members',
                        ...
                        organisation.members = membersResult.team._id;
                        organisation.owners = ownersResult.team._id;

                        organisation.save(function (err, orgSaveResult) {
                            if (err || !orgSaveResult) {
                                return self.emit(eventType.invalid, app);
                            }
                            app.organisation = orgSaveResult;
                            app.user.teams.push(orgSaveResult.owners);
                            app.user.teams.push(orgSaveResult.members);
                            app.user.organisations.push(orgSaveResult._id);
